$( document ).ready(function() {
	...
	queryParam = window.location.href.split('skill=')[1];
	...
	chart: {
		...
		yAxis: {
	        gridLineWidth: 0.75,
	        title: {
	            text: 'Number of jobs that use ' + jobTitle 
	        }
	    },
		series: [],
		legend: {
	...
	chart: {
		...
		xAxis: {
			...
			labels: {
				...
				formatter: function() {
					return '<a href="' + window.location.href.split('skills')[0] + 'jobs?title=' + this.value + '">' + this.value + '</a>';
				}
			}
		},
		yAxis: {
	        ...
	        // min: 0,
	        title: {
	            text: 'Percent of jobs that use ' + jobTitle
	        },
	        stackLabels: {
	        	enabled: true,
	        	formatter: function () {
	...
	function makeIndustryChart(jobTitle){
		$.getJSON('data/industries_by_skill.json', function(data){
			data = data[jobTitle];
			data = _.pairs(data);
	...
	function makeSkillsChart(jobTitle){
		$.getJSON('data/skills_job.json', function(data){
			// get all the objects from that title
			data = _.where(data, {title:jobTitle});
			// sort the skills
			var skillData = {};
			_.each(data, function(item){
				// console.log(_.reduce(_.values(_.omit(data,['title', 'skill']), function(memo, num){return memo + num;},0)));
				skillData[item.skill] = _.reduce(_.values(_.omit(item,['title', 'skill'])), function(memo, num){return memo + num;},0);
				// console.log(_.reduce(_.values(_.omit(item,['title', 'skill'])), function(memo, num){return memo + num;},0));
			});
	...
	function makeRelatedChart(jobTitle) {
		...
		$.getJSON('data/related_skills.json', function(data){
			data = _.where(data,{title:jobTitle});
			// console.log(data);
		    ...
		    var svg = d3.select("#related-skills-plot")
						.append("svg")
						.attr("width", w)
						.attr("height", h)
						.attr("margin-right", 500)
						.attr("margin-top", 200);
		    ...
		    var node = gnodes.append("a")
		    	.attr("xlink:href", function(d){return window.location.href.split('?')[0] + '?skill=' + d.name;})
		    	.append("circle")
		    	.attr("class","node")
			    // .attr("cx", function(d) { return d.x; })
		     //  	.attr("cy", function(d) { return d.y; })
